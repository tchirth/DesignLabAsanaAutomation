...
steps.get_gids.matches_s2p has a key, value pair matching section's gids to the corresponding subproject's gid. This matching is done each day when the corresponding database workflow refreshes the databse. This is when we sync tasks from sections to their corresponding subproject.
